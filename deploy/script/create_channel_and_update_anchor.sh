...
peer channel create -o orderer.gtbcsf.com:7050 -c sfchl -f /Users/ywt/fabric-run-env/config/channel-artifacts/channel.tx --outputBlock /Users/ywt/fabric-run-env/config/channel-artifacts/sfchl.block --tls --cafile /Users/ywt/fabric-run-env/config/crypto-config/ordererOrganizations/gtbcsf.com/orderers/orderer.gtbcsf.com/msp/tlscacerts/tlsca.gtbcsf.com-cert.pem
if [ $? -ne 0 ]; then
  echo "ERROR !!!! Unable to create channel: $channel_name"
  exit 1
fi

peer channel join -b /Users/ywt/fabric-run-env/config/channel-artifacts/sfchl.block --tls --cafile /Users/ywt/fabric-run-env/config/crypto-config/ordererOrganizations/gtbcsf.com/orderers/orderer.gtbcsf.com/msp/tlscacerts/tlsca.gtbcsf.com-cert.pem
if [ $? -ne 0 ]; then
  echo "ERROR !!!! Peer[$peer_host_dns] unable to join channel[$channel_name]"
  exit 1
fi

peer channel update -o orderer.gtbcsf.com:7050 -c sfchl -f /Users/ywt/fabric-run-env/config/channel-artifacts/CoreEnterpriseMSP_anchors.tx --tls --cafile /Users/ywt/fabric-run-env/config/crypto-config/ordererOrganizations/gtbcsf.com/orderers/orderer.gtbcsf.com/msp/tlscacerts/tlsca.gtbcsf.com-cert.pem
if [ $? -ne 0 ]; then
